...
def about(update, ctx):
    ...
    if timestart < int(timestamp):
        if update.message.chat.type == "private":
            ctx.bot.send_message(chat_id=update.message.chat_id,
                                 text="Hello there, "
                                 "I am the Bitweb Telegram Tipbot\\. Run /help to see my full list of commands\\. "
                                 "This bot is fully [Open Source](https://github\\.com/bitweb\\-project/BitwebTGBot)\\."
                                 , parse_mode="MarkdownV2")
        else:
            ctx.bot.send_message(chat_id=update.message.chat_id,
...
def price(update, ctx):
    ...
    if timestart < int(timestamp):
        ...
        usd_vol_24h = str(format(price[config.coin['coin_name']]["usd_24h_vol"],'.7f'))
...
""", parse_mode="HTML", disable_web_page_preview=1)
        ...
        usd2 = str(format(float(price2["price_usd"]),'.7f'))
        volume_usd_24h = str(format(float(price2["volume_24h_usd"]),'.7f'))

        ctx.bot.send_message(chat_id=update.message.chat_id, text=f"""
<a href="coinpaprika.com/coin/bte-bitweb/#!exchanges">Coinpaprika</a>
        ...
        if not db.checkUser(user["id"]):
            ...
            if target is not None:
                if not db.getUserID(target):
                    ctx.bot.send_message(chat_id=update.message.chat_id,
                                         text="Oops, looks like your sending to a user who hasn't registered or has changed their username. \nAsk them to do /help to register/re-register!\nPlease be mindful that usernames are case senstive. \nOr are you trying to send to yourself or a bot. \nMake sure that the case of the target is correct.")
                else:
